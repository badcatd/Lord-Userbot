...
# ========================= CONSTANTS ============================
AFKSTR = [
    f"**Maaf Lebah {ALIVE_NAME} Sedang AFK!**",
    f"**Maaf Lebah {ALIVE_NAME} Sedang AFK\n Tunggu Sampai Dia Kembali Online!**",
    f"**Lebah {ALIVE_NAME} Sedang AFK\n Tunggulah Sampai Online**",
    f"**Maaf Lebah {ALIVE_NAME} Sedang AFK!**",
]
    ...
    if mention.message.mentioned and not (await mention.get_sender()).bot:
        if ISAFK:
            ...
            if mention.sender_id not in USERS:
                if AFKREASON:
                    await mention.reply(f"**𓆤 Lebah {ALIVE_NAME} Lagi% AFK** {afk_since} **Yang Lalu.**\
                        \n➠ **Alasan:** `{AFKREASON}`")
                else:
                ...
                if USERS[mention.sender_id] % randint(2, 4) == 0:
                    if AFKREASON:
                        await mention.reply(f"**𓆤 Lebah Masih AFK** {afk_since} **Yang Lalu.**\
                            \n➠ **Alasan:** `{AFKREASON}`")
                    else:
                        await mention.reply(str(choice(AFKSTR)))
    ...
    if sender.is_private and sender.sender_id != 777000 and not (
        ...
        if apprv and ISAFK:
            ...
            if sender.sender_id not in USERS:
                if AFKREASON:
                    await sender.reply(f"𓆤 **Lebah Sedang AFK** {afk_since} **Yang Lalu**.\
                        \n➠ **Alasan**: `{AFKREASON}`")
                else:
